...
cellIndexProd1 = sub2ind([Nx, Ny, Nz], 5, 5, 3);
cellIndexInj1 = sub2ind([Nx, Ny, Nz], 10, 10, 3);
cellIndexInj2 = sub2ind([Nx, Ny, Nz], 10, 1, 3);
cellIndexInj3 = sub2ind([Nx, Ny, Nz], 1, 10, 3);
cellIndexInj4 = sub2ind([Nx, Ny, Nz], 10, 10, 3);
...
pressureInj2  = zeros(numSteps, 1);
pressureInj3  = zeros(numSteps, 1);
pressureInj4  = zeros(numSteps, 1);

sWProd1 = zeros(numSteps, 1);
sWInj1  = zeros(numSteps, 1);
sWInj2  = zeros(numSteps, 1);
sWInj3  = zeros(numSteps, 1);
sWInj4  = zeros(numSteps, 1);
...
for t = 1:numSteps
    ...
    pressureInj2(t)  = st.pressure(cellIndexInj2);
    pressureInj3(t)  = st.pressure(cellIndexInj3);
    pressureInj4(t)  = st.pressure(cellIndexInj4);

    % Saturasi air (kolom pertama)
    sWProd1(t) = st.s(cellIndexProd1, 1);
    sWInj1(t)  = st.s(cellIndexInj1, 1);
    sWInj2(t)  = st.s(cellIndexInj2, 1);
    sWInj3(t)  = st.s(cellIndexInj3, 1);
    sWInj4(t)  = st.s(cellIndexInj4, 1);
end
...
% Buka file output
fid_p  = fopen('A2M Pressure.txt', 'w');
fid_sw = fopen('A2M Water Saturation.txt', 'w');
fid_so = fopen('A2M Oil Saturation.txt', 'w');
fid_sg = fopen('A2M Gas Saturation.txt', 'w');
