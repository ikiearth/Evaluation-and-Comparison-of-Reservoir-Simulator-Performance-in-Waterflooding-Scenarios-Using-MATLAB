...
W = verticalWell(W, G, rock, ...
    ...
    'val', 0, ...                  % Injection rate (volume per second)
    'type', 'rate', ...                  % Control type: Rate
...
simTimeProd = 0.5 * day;     % Total simulation time for production (10 years)
simTimeInj = 0.5 * day;      % Total simulation time for injection
nstep   = 0.02083333 * day;  % Number of time steps (60 days)
refine  = 0;                 % Refinement factor for time steps

% Definisikan timestep untuk produksi dan injeksi menggunakan fungsi ramp-up
dt = rampupTimesteps(1 * day, nstep, refine);
...
%         W2(i).val = 0;        % Set injection rate to predefined RATEWATER
%         W2(i).type = 'rate';        % Use rate control mode
...
% Buka file output
fid_p  = fopen('A1M-INIT Pressure.txt', 'w');
fid_sw = fopen('A1M-INIT Water Saturation.txt', 'w');
fid_so = fopen('A1M Oil-INIT Saturation.txt', 'w');
fid_sg = fopen('A1M Gas-INIT Saturation.txt', 'w');
