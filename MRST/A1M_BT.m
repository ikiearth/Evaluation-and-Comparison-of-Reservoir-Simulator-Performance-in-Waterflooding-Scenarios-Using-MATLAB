...
mrstModule add ad-core ad-blackoil ad-props mrst-gui linearsolvers
close all;
...
W = verticalWell(W, G, rock, ...
    1, 1, 1:3, ...                      % Location: row (I=1), column (J=1), full perforation (K=[])
...
simTimeInj = 19 * year;       % Total simulation time for injection
...
linSolver = BackslashSolverAD();
nls = NonLinearSolver('useRelaxation', true, 'linearSolver', linSolver);
...
[wellSols, states, report] = simulateScheduleAD(state, model, schedule, ...
    'nonlinearsolver', solver, ...           % Use nonlinear solver
...
cellIndexInj1 = sub2ind([Nx, Ny, Nz], 10, 10, 3);
...
for t = 1:numSteps
    ...
    % Ambil saturasi air (kolom ke-1 dari s)
...
% Buka file output
fid_p  = fopen('A1M Pressure.txt', 'w');
fid_sw = fopen('A1M Water Saturation.txt', 'w');
fid_so = fopen('A1M Oil Saturation.txt', 'w');
fid_sg = fopen('A1M Gas Saturation.txt', 'w');
...
%%
step = 132;
state_Peaceman = states{step};         % kalau states disimpan sebagai cell array

wi   = W(1).WI;                        % vector WI untuk setiap cell di sumur
cell = W(1).cells;                     % cell yang ditembus sumur
pgrid = state_Peaceman.pressure(cell); % tekanan di grid cell
pwf   = W(1).val;                      % bottom-hole pressure yang ditetapkan

% Hitung flow rate dengan rumus Peaceman (manual verifikasi)
q_calc = wi .* (pgrid - pwf);
q_mrst = state_Peaceman.wellSol(1).flux;  % total laju aliran ke sumur

fprintf("Manual q = %.8f STB/day\n", sum(q_calc) * 86400 * 6.2898); % m³/s → STB/day
fprintf('MRST   q = %.8f STB/day\n', sum(q_mrst) * 86400 * 6.2898); % m³/s → STB/day